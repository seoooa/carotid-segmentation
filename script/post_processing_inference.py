...
from src.data.dataloader import CarotidDataModule
...
class CarotidSegmentModel(pytorch_lightning.LightningModule):
    ...
    def __init__(
        ...
        self.post_pred_with_largest_cc = Compose(
            [
    ...
    def save_result(self, inputs, outputs, labels, filename_prefix="result"):
        ...
        inputs_nifti = nib.Nifti1Image(
            inputs_np,
            np.array([[0.6, 0, 0, 0], [0, 0.6, 0, 0], [0, 0, 2.0, 0], [0, 0, 0, 1]]),
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Original outputs without post-processing for saving
        outputs_original = [self.post_pred(i) for i in decollate_batch(outputs)]
        
        # Post-processed outputs with KeepLargestConnectedComponent
        outputs_processed = [self.post_pred_with_largest_cc(i) for i in decollate_batch(outputs)]
        labels = [self.post_label(i) for i in decollate_batch(labels)]
        ...
        self.save_result(
            images, outputs_processed[0], labels[0], filename_prefix=f"Subj_{patient_id}"
        )

        # Calculate metrics using post-processed outputs
        MetricFactory.calculate_metrics(self.metrics, outputs_processed, labels)
...
def main(
    ...
    log_dir = f"result/post_processing/{arch_name}/{target}"
    ...
    data_module = CarotidDataModule(
        data_dir=f"data/Han_Seg_{target.capitalize()}_pp",
        batch_size=1,
        patch_size=(96, 96, 96),
        num_workers=2,
        cache_rate=0.1,
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        ...
        if "final_model.ckpt" in checkpoint_filename:
            ...
            model = CarotidSegmentModel.load_from_checkpoint(
                checkpoint_path,
                arch_name=arch_name,
                loss_fn=loss_fn,
                batch_size=1,
