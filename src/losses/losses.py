...
from src.losses.cldice import SoftDiceclDiceLoss
import torch
import warnings
...
#         # CL-Dice Loss 계산 (배경 채널 제외)
#         input_fg = input[:, start_channel:, ...]  # 배경 제외
#         target_fg = target[:, start_channel:, ...]  # 배경 제외
        
#         # 배경을 제외한 채널에 대해서만 skeleton 계산
#         skel_pred = soft_skel(input_fg, self.iter)
#         skel_true = soft_skel(target_fg, self.iter)
        
#         tprec = (torch.sum(torch.multiply(skel_pred, target_fg)) + self.smooth) / (
#             torch.sum(skel_pred) + self.smooth
#         )
#         tsens = (torch.sum(torch.multiply(skel_true, input_fg)) + self.smooth) / (
...
class LossFactory:
    ...
    def create_loss(loss_name):
        if loss_name == "DiceLoss":
            ...
            return SoftDiceclDiceLoss(to_onehot_y=True, softmax=True)
        else:
